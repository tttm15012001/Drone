...
void set_destination(float x, float y, float z, float psi)
{
	...
	ROS_INFO("%f", deg2rad);
	ROS_INFO("X: %f - %f - %f - %f - %f - %f", x, Xlocal, correction_vector_g.position.x, local_offset_pose_g.x, correction_heading_g, local_offset_g);
	ROS_INFO("Y: %f - %f - %f - %f - %f - %f", y, Ylocal, correction_vector_g.position.y, local_offset_pose_g.y, correction_heading_g, local_offset_g);
	ROS_INFO("Z: %f - %f - %f - %f - %f - %f", z, Zlocal, correction_vector_g.position.z, local_offset_pose_g.z, correction_heading_g, local_offset_g);
...
float *splitWayPointFromMsg(float* &arr, char msg[100]) {
	const char* delimiter = " ";
	char* param = std::strtok(msg, delimiter);
	int count = 0;
	arr[count] = std::stof(param);
	while(param != nullptr) {
		count++;
		param = std::strtok(nullptr, delimiter);
		if(param != nullptr) {
			arr[count] = std::stof(param);
		}
	}
...
int init_publisher_subscriber(ros::NodeHandle controlnode)
{
	...
	local_pos_pub = controlnode.advertise<geometry_msgs::PoseStamped>((ros_namespace + "/mavros/setpoint_position/local").c_str(), 1000);
