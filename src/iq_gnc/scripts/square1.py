...
def callback(data):
    global canReceive
    if canReceive == True or data.data.find('drone2: ') != -1:
        ...
        if(data.data == 'land'):
            ...
            if analizeData[0] == 'takeoff':
                if 'drone2True' in analizeData[1]:
                    drone.takeoff(3)
                ...
                drone.set_destination(x=float(coordinates_arr[0]), y=float("-2"), z=float(coordinates_arr[2]), psi=0)
        canReceive = False
